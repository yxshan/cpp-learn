...
> JavaScript `Map` 按插入顺序迭代，并按语言规定的键相等语义处理对象身份与数值；
> `std::unordered_map` 不承诺遍历顺序，调用者还可提供 `Hash` 与 `Pred`，两者必须保持
> 一致。JS 对象字面量也不是直接替代品：C++ 键和值类型在编译期固定，插入和 rehash
> 还具有明确的迭代器与引用失效规则。

## 相关内容

`<unordered_map>` 声明该类型及相关辅助接口；需要按键排序、范围查询或稳定键序遍历时，
比较 `std::map`。
