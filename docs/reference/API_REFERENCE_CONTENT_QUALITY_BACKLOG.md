...
The active catalog contains 66 Entries. All have at least one primary source,
and all 118 examples pass the local toolchain gate. Depth still varies by
...
Quality batches 1 through 3 and breadth batches 4 through 7 are complete. The existing
...
The kind-aware audit currently checks 62 of 66 Entries; three landing Entries
...
The current 66 Entries remain in scope. The following 54 candidates make the
...
### 4.5 Utilities and callable objects: 5 candidates

- `std::tuple`, `std::variant`, `std::any`, `std::expected`.
- `std::function`.
...
4. Add string conversion, shared ownership, and utility vocabulary.
   **String conversion and shared ownership completed in breadth batch 6;
   foundational utility vocabulary completed in breadth batch 7; callable and
   sum-type vocabulary remains.**
