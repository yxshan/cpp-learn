...
`std::sort` 不改变范围长度，也不让容器重新分配存储，因此它本身不会使仍有效的迭代器、
指针或引用变成不可解引用。不过算法会在既有位置之间交换或移动值：旧迭代器仍指向同一
位置，却不保证仍观察到排序前的那个逻辑元素。若比较器修改容器结构、销毁元素，或让参与
比较的引用失效，则违反调用前提。
...
C++ `std::sort` 与 JavaScript `Array.prototype.sort()` 都会原地修改序列，但默认比较语义不同：
前者按元素的 `<` 关系排序，后者默认先把元素转换为字符串再比较。JavaScript 比较函数通常
返回负数、零或正数；C++ 比较器返回 `bool`，并且必须建立严格弱序。`std::sort` 不保证等价
元素保持原相对顺序，需要稳定性时应改用 `std::stable_sort`。

## 相关内容

查看 `<algorithm>` 了解算法头文件边界，查看 `std::vector` 了解示例中的连续容器。
