...
算法不拥有输入或目标对象。输入迭代器、N 个目标写入位置及其元素必须在对应访问完成前保
持有效；返回的输出尾只在目标对象仍存活且没有按容器规则失效时可用。`std::copy` 本身不
改变普通固定目标的结构，但 `back_inserter` 会调用容器插入操作，可能扩容并使该容器的旧
迭代器、指针和引用失效。输入与输出使用同一容器时必须遵守上文与具体重载对应的重叠规
则，并保证算法尚未使用的迭代器不会因写入而失效。
