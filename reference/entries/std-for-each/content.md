...
算法不拥有范围、函数对象引用、projection 或回调捕获的对象，也不会延长它们的生命周期。
所有输入迭代器和捕获引用必须在本次调用期间有效。算法本身不改变范围大小；允许的当前元
素赋值通常不使容器结构失效，但在回调中向同一 vector 插入等结构修改可能立即使算法持有
的迭代器失效。经典重载返回的函数对象是一个值，其内部若仍保存引用，引用目标仍须存活。
