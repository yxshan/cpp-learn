...
- `first`、`last`：待压紧的有效前向半开范围。
- `pred`：对元素可调用的一元谓词；返回 `true` 表示该元素应从逻辑结果中移除。

元素必须可移动赋值，因为保留值可能被移动到更靠前的位置。谓词不能修改被传入对象，也不
能通过容器结构修改使当前范围失效。算法要求的是可写前向范围，不会为目标容器自动调用成
员 `erase`。
...
返回的 `new_end` 是保留范围的新尾后位置。若没有元素匹配，返回原 `last`；若全部匹配，
返回 `first`。它不会改变容器的 `size()`，也不代表尾段对象已经销毁。
