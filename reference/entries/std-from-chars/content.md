...
协议字段、配置片段或日志 token 已经位于明确字符区间中，并且调用者希望避免区域设置、异常和
临时拥有型字符串时使用。需要自动跳过空白、接受本地化格式或产生面向用户的详细诊断时，应先
在更高层定义语法和错误模型，而不是把 `from_chars` 当成完整表单解析器。

## 参数与整数解析规则

- `first`、`last`：同一有效字符范围的首尾指针，解析只读取 `[first, last)`。
- `value`：成功时写入的目标数值；无匹配或超出范围时保持原值。
- `base`：整数重载的进制，必须位于 2 到 36。
- `format`：浮点重载接受的 `chars_format` 组合，决定允许的表示形式。
...
底层字符区间必须在完整调用期间有效，且 `first`、`last` 必须表示同一有效范围。返回的
`ptr` 指向该原始范围内部或尾后位置；只要调用者还要解引用它或由它建立 `string_view`，底层
字符存储就必须继续存活且未被重新分配。结果对象不拥有输入，错误码也无法修复悬空指针或倒置
区间。写入的数值是独立值，不再借用字符范围。
