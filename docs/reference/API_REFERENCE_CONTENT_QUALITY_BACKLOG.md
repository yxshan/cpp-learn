...
The active catalog contains 114 Entries. All have at least one primary source,
and all 214 examples pass the local toolchain gate. Depth still varies by
...
Quality batches 1 through 3 and breadth batches 4 through 16 are complete. The
...
The kind-aware audit currently checks 110 of 114 Entries; three landing Entries
...
The current 114 Entries remain in scope. The following 6 candidates make the
...
### 4.9 Concurrency: 9 delivered, 6 candidates
...
6. Add concurrency last, after its nondeterministic-example and memory-model
   review requirements are explicit. **In progress:** breadth batches 15 and 16
   deliver thread/mutex foundations, movable and multi-lock ownership, and
   condition-variable waiting; six candidates remain, including a toolchain-
