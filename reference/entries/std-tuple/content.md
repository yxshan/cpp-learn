...
值元素由 tuple 拥有，tuple 销毁时才结束这些元素子对象的生命周期。普通 tuple 赋值是逐元素
赋值，swap 是逐元素交换；它们不会重建元素子对象，已有元素引用仍指向原来的成员身份。不过，
元素持有的值和内部资源会改变，其中指针、迭代器或视图是否失效仍由该元素类型的赋值/swap 合同
决定。

`tuple<T&...>`、`tie` 和 `forward_as_tuple` 只保存引用，不延长实参生命周期；尤其不要保存包含
临时量引用的 `forward_as_tuple(...)` 结果。
