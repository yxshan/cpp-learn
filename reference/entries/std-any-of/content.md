...
- `first`、`last`：经典重载检查的有效输入半开范围。
- `pred`：对解引用后的元素可调用并产生布尔语境结果的一元谓词。
- `policy`：仅执行策略重载存在；它改变调度、异常和函数对象并发要求。
- `proj`：仅 ranges 家族存在，在谓词之前把元素投影成待检查值。

谓词不得通过参数修改元素，也不得对正在遍历的容器执行会使迭代器失效的结构修改。ranges
重载还要求投影和谓词满足声明中的间接调用约束。
...
算法不拥有范围、谓词、投影或它们捕获的对象，也不会延长任何生命周期。输入范围、迭代器
和捕获引用必须在调用结束前持续有效。算法本身不改变容器结构；若谓词通过其他别名修改容
器并使迭代器失效，后续行为不再满足调用前提。返回的 `bool` 是独立值，不借用范围。
