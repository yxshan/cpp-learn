...
- `first`、`last`：待压缩的有效、可写前向半开范围。
- `pred`：经典自定义重载判断相邻元素是否等价的二元谓词。
- `policy`：仅执行策略重载存在，改变调度、异常和函数对象并发要求。
- `comp`、`proj`：ranges 家族的等价关系和投影。

范围元素必须可移动赋值；自定义判断必须形成等价关系，并且不能修改参数或通过结构修改使
当前范围失效。相同含义的等价判断应在整个调用期间保持稳定。
...
- ranges 接口返回 `{new_end, original_end}` 组成的 subrange。
- `[new_end, last)` 中对象仍存在，但值不受结果合同保证；不要读取它们推断结果。
- 容器的 `size()` 不会变化。

空范围与单元素范围都返回无需进一步压缩的逻辑边界。返回迭代器只描述有效前缀，不会自动
销毁尾段对象或改变容器大小。

## 复杂度

设范围长度为 N。N 为 0 时不调用等价判断；非空范围恰好执行 N−1 次相邻比较，并进行至多
线性数量的移动赋值。复杂度是线性的，但用户谓词、projection 和元素移动各自的成本仍计入
实际运行时间。
