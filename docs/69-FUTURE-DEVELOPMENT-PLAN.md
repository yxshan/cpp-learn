...
本计划把架构审计转成可由后续 AI 模型逐项执行的开发切片。顺序强调风险和依赖：先统一内容政策，再重构作者生命周期，然后修正共享呈现和应用局部性，最后加速内容扩展。
...
| 1 | Reference content policy | 检查、模板和修复共享一个类型化政策 | 0 |
| 2 | Authoring 生命周期深化 | 16 操作兼容门面背后形成清晰内部模块 | 1 |
| 3 | Reference presentation | CLI 不再依赖 Web app | 0，可与 2 独立 |
| 4 | A3 真实批次测量 | 证明速度或定位真实瓶颈 | 1，建议 2 后 |
| 5 | Server/Web 局部性 | 降低大文件的无关认知负担 | 3 |
| 6 | 内容持续扩展 | 用稳定快速流水线补齐高价值 API | 1、4 |
...
## 4. P1：统一 Reference content policy

这是最高优先级，因为相同语义目前分散在 Authoring profile、质量脚本和 repair target map 中，已经发生过 Header finding 无法进入修复计划的问题。
...
## 5. P2：深化 Reference Authoring 生命周期
...
## 6. P3：共享 Reference presentation
...
## 8. P5：Server 与 Web 局部性

### P5-1 Server route groups
...
只有 P4 证明人审界面是瓶颈时才开发。第一版范围限定为：风险队列、draft/published diff、claim/source 证据、编译结果、渲染预览和 publish plan 确认。
